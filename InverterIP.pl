...
# AS AT 3June2014
#
# inverter.pl - polls data from the RS232 or IP port on certain inverters and outputs data to a csv file
...
#       perl inverter.pl "192.168.178.74"
#
# Arguments:
...
# + editions by eevers:
#          + added ip config
#		   + added VPV1 and VPV2 (panel voltage) to output file
#		   + added (average) voltage to pvoutput
#		   + changed Etoday in file from kWh to Wh in order to make it easier to import in excel
#######################################################################
#
...
$config->define( "secs_fileoutput_freq=s");
...
$config->define( "hex_ipdata_to_follow_index=s" );
$config->define( "hex_ipcapacity_index=s"  );
$config->define( "hex_ipcapacity_length=s" );
$config->define( "hex_ipfirmware_index=s"  );
$config->define( "hex_ipfirmware_length=s" );
$config->define( "hex_ipmodel_index=s"     );
$config->define( "hex_ipmodel_length=s"    );
$config->define( "hex_ipmanuf_index=s"     );
$config->define( "hex_ipmanuf_length=s"    );
$config->define( "hex_ipserial_index=s"    );
$config->define( "hex_ipserial_length=s"   );
$config->define( "hex_ipother_index=s"     );
$config->define( "hex_ipother_length=s"    );
$config->define( "hex_ipconfserial_index=s" );
...
$config->define( "ipsendhex_broadcast=s"    );
$config->define( "ipsendhex_serial=s"       );
$config->define( "ipsendhex_step2=s"     );
$config->define( "ipsendhex_step3=s"     );
$config->define( "ipsendhex_data=s"      );
$config->define( "ipsendhex_step5=s"     );
...
$config->define( "iprecvhex_serial=s"      );
$config->define( "iprecvhex_step2=s" );
$config->define( "iprecvhex_step3=s"     );
$config->define( "iprecvhex_data=s"    );
$config->define( "iprecvhex_step5=s"       );
...
# check if we will use serialport or IP
    if ($port =~ m/IP:/i) {   # IP is found in the argument
    $ipport=1;  # remember that we communicate on the IP port; 

# first remove the IP: from the argument
    my ($IPaddress,$UDPbroadcast);
    $IPaddress=substr($port,3);
    print "with inverter at $IPaddress ";
# then replace the 4th part of the IP address with 255
    $UDPbroadcast= substr($IPaddress,0,rindex($IPaddress,'.')) . '.255';
    print "and broadcast at $UDPbroadcast\n";
    
# Open tcp port, than udp port and broadcast udp pakket with init string
# than listen on TCP socket 

     $udpsock = IO::Socket::INET->new (
			LocalPort => 58428,
			Type	  => SOCK_DGRAM,
			Proto     => 'udp'
		 ) || die 'ERROR in UDP Socket Creation : $@\n';

	$udpsock->sockopt(SO_REUSEADDR, 1) || die 'Error setsockopt: $!\n';
	$udpsock->sockopt(SO_BROADCAST, 1) || die 'Error setsockopt: $!\n';
	$udpsock->autoflush(1);
# next line is an undocumented feature, but tcpdump/wireshark shows the correct package
    my $udp_handle;
	$udp_handle=$udpsock->connect(1300, inet_aton($UDPbroadcast)) || die 'Error UDP socket connect: $!\n';
 

    my $UDPstr = &convHexToRaw($config->ipsendhex_broadcast);
#    print "\nGoing to write UDP broadcast: $UDPstr \n";
    print $udpsock "$UDPstr";
    print "UDP broadcast done, ";  
    close ($udpsock);
  
    $tcpsock = IO::Socket::INET->new (
# don' use localhost otherwise we will only be able to communicate with our host
# 			LocalHost => 'localhost',
			PeerAddr  => $IPaddress,
#			PeerPort  => 4099,
			LocalPort => 1200,
			...
			ReuseAddr => 1,
			Listen    => 5
		) || die 'ERROR in TCP Socket Creation : $@\n';
   	    $tcpsock->autoflush(1);

	print "now waiting for TCP connection from inverter\n"; 
    
	if ( $tcphandle = $tcpsock->accept() )
	{
	
		$tcphandle->autoflush(1);	# according perldoc.perl.org to circumvent buffering
	
    	my ($peeraddress,$peerport);
    	$peeraddress=$tcphandle->peerhost();
    	$peerport=$tcphandle->peerport();

	    print "Inverter at address: $peeraddress, port: $peerport responded\n";
	    sleep(1);

	} else
	{
		print "TCP connection from inverter timed-out\n";
		die 'TCP connection setup failed: ';
	}
	
   } else   # not IP port but serial port
   {
...
#######################################################################
#
# Set lastFileoutputTime & nextFileoutputTime
#
sub setFileoutputTimes() {
  $lastFileoutputTime = time;
  $nextFileoutputTime = $lastFileoutputTime + $config->secs_Fileoutput_freq;
} #end setFileoutputTimes
...
#######################################################################
#
# Return 1 (true) if time for next file write, 0 (false) if not.
#
sub isNextFileoutputDue() {
  if ( $lastFileoutputTime == 0 || $nextFileoutputTime <= time ) {
    return 1;
  }
  return 0;
} #end isNextFileoutputDue
...
sub writeToPort() {
  ...
  my $countOut;
  if ($ipport){ 
  	$countOut=$tcphandle->send($writeStr);
  } else
  {
  $countOut = $serial->write($writeStr);
...
sub writeReadBuffer() {
  ...
  if($ipport) {
		$countIn = $tcphandle->recv($stringIn,170); # will read _up to_ 170 chars
      	$buffer = $stringIn;
  ...
  } else
  {
	while ( $timeout > 0 ) {
    	...
    	($countIn,$stringIn) = $serial->read(255); 		# will read _up to_ 255 chars
    	if ($countIn > 0) {
...
sub calcReqConfSerial() {
  ...
  my $hexSerial = "";
  if($ipport){
	  $hexSerialTemp=substr($hexStr, $config->hex_ipconfserial_index, $config->hex_ipserial_length);
	  $hexSerial=trim($hexSerialTemp);
  } else
  {
	  $hexSerial=substr($hexStr, $config->hex_confserial_index, $config->hex_serial_length);
  }
...
sub initialiseInverter() {
  ...
  my $rawRequest="";
  my $hexResponse="";
  my $confSerialRequest="";
  my $hexResponse2=""; 
  
  if($ipport){  # ip port

  #
  # IP step 1: Request serial number
  #
    print "Send -> req serial: " . $config->ipsendhex_serial . "\n";
  	$hexResponse = &writeReadBuffer($config->ipsendhex_serial,$config->iprecvhex_serial);

	if ( $hexResponse ne "" ) {
	$hexResponse2=$hexResponse;
    $confSerialRequest = &calcReqConfSerial($hexResponse);
	&parseVersData($hexResponse2);
  ...
  #
  # Serial step 1: Start initial handshake with inverter (reset network)
  #

  	$rawRequest = &convHexToRaw($config->sendhex_initialise);
  	print "Send -> req init inverter: " . $config->sendhex_initialise . "\n";
  
    &writeToPort($rawRequest);
  #
  # Serial step 2: request the serial number (query network)
  #
    print "Send -> req serial: " . $config->sendhex_serial . "\n";
  	$hexResponse = &writeReadBuffer($config->sendhex_serial,$config->recvhex_serial);
  
  #
  # Serial step 3: confirm the serial number
  #
  if ( $hexResponse ne "" ) {
    $confSerialRequest = &calcReqConfSerial($hexResponse);
    print "Send -> confirm serial: $confSerialRequest \n";
    $hexResponse2 = &writeReadBuffer($confSerialRequest,$config->recvhex_conf_serial);
  	}
  }
...
sub parseVersData() {
  ...
  #
 if($ipport) {
	if ( $config->hex_ipcapacity_length > 0 && $config->hex_ipcapacity_index + $config->hex_ipcapacity_length < $hexLength ) {
	    	$HASH{CAPACITY} = &trim( pack ("H*", substr($hexData, $config->hex_ipcapacity_index, $config->hex_ipcapacity_length)) );
  	}
  	if ( $config->hex_ipfirmware_length > 0 && $config->hex_ipfirmware_index + $config->hex_ipfirmware_length < $hexLength ) {
    	$HASH{FIRMWARE} = &trim( pack ("H*", substr($hexData, $config->hex_ipfirmware_index, $config->hex_ipfirmware_length)) );
  	}
  	if ( $config->hex_ipmodel_length > 0 && $config->hex_ipmodel_index + $config->hex_ipmodel_length < $hexLength ) {
    	$HASH{MODEL} = &trim( pack ("H*", substr($hexData, $config->hex_ipmodel_index, $config->hex_ipmodel_length)) );
  	}
  	if ( $config->hex_ipmanuf_length > 0 && $config->hex_ipmanuf_index + $config->hex_ipmanuf_length < $hexLength ) {
    	$HASH{MANUF} = &trim( pack ("H*", substr($hexData, $config->hex_ipmanuf_index, $config->hex_ipmanuf_length)) );
  	}
  	if ( $config->hex_ipserial_index > 0 && $config->hex_ipserial_index + $config->hex_ipserial_length < $hexLength ) {
    	$HASH{SERIAL} = &trim( pack ("H*", substr($hexData, $config->hex_ipserial_index, $config->hex_ipserial_length)) );
  	}
  	if ( $config->hex_ipother_length > 0 && $config->hex_ipother_index + $config->hex_ipother_length < $hexLength ) {
    	$HASH{OTHER} = &trim( pack ("H*", substr($hexData, $config->hex_ipother_index, $config->hex_ipother_length)) );
  	}
  } else
  {	
...
sub parseDataFmt() {
  ...
  if($ipport) {
  	$dataOffset = $config->hex_ipdata_to_follow_index + 1;
  	$dataToFollow = hex($d[$config->hex_ipdata_to_follow_index]);
  	print "dataToFollow = hex($d[$config->hex_ipdata_to_follow_index]) = $dataToFollow\n";  
  } else
  {
  	$dataOffset = $config->hex_data_to_follow_index + 1;
  	$dataToFollow = hex($d[$config->hex_data_to_follow_index]);
  	print "dataToFollow = hex($d[$config->hex_data_to_follow_index]) = $dataToFollow\n";
  }
...
sub parseData() {
  ...
  if($ipport) {
  	$dataToFollow= hex( substr( $hexData, $config->hex_ipdata_to_follow_index*2, 2 ) );
  	$startIndex = ( $config->hex_ipdata_to_follow_index + 1 )*2;
  } else
  {
  	$dataToFollow= hex( substr( $hexData, $config->hex_data_to_follow_index*2, 2 ) );
  	$startIndex = ( $config->hex_data_to_follow_index + 1 )*2;
  }
  my $numOfChars = $dataToFollow * 2;
  my $data = substr( $hexData, $startIndex, $numOfChars );
...
#      print LOGFILE "DATE,TIMESTAMP,TEMP,VPV,IAC,VAC,FAC,PAC,ETOTAL,HTOTAL,MODE,ETODAY\n";
      print LOGFILE "DATE,TIMESTAMP,TEMP,VPV,VPV1,VPV2,IAC,VAC,FAC,PAC,ETOTAL,HTOTAL,MODE,ETODAY" .
                    ",ETOTALH,HTOTALH,ERR_GV,ERR_GF,ERR_GZ,ERR_TEMP,ERR_PV1,ERR_GFC1,ERR_MODE,UNK10\n";
    }

    my $etotal = ($HoH{ETOTALL}{VALUE} + $HoH{ETOTALH}{VALUE});
    my $htotal = ($HoH{HTOTALL}{VALUE} + $HoH{HTOTALH}{VALUE});
    my $etoday = ($HoH{ETODAY}{VALUE}*1000);
    if ($config->flags_debug) {
...
sub main() {
   ...
   #
   # Initialise Poll, Fileoutput & Pvoutput times
   #
   &setPollTimes();
   &setFileoutputTimes();
   &setPvoutputTimes();
   ...
   my $voltageAVG=0;
	...
	if ($ipport) { # IP port
	#
	# IP Step 2   Data Format
	#
	    print "Send -> step2: " . $config->ipsendhex_step2 . "\n";
   		$hexResponse = &writeReadBuffer($config->ipsendhex_step2,$config->iprecvhex_step2);
	    &parseDataFmt($hexResponse) if ($hexResponse ne "");
   		
   	#
	# IP Step 3
	#
	    print "Send -> step3: " . $config->ipsendhex_step3 . "\n";
   		$hexResponse = &writeReadBuffer($config->ipsendhex_step3,$config->iprecvhex_step3);

	#
	# IP data
	#
	    print "Send -> data: " . $config->ipsendhex_data . "\n";
   		$hexResponse = &writeReadBuffer($config->ipsendhex_data,$config->iprecvhex_data);

	#
	# IP step 5
	#
	    print "Send -> step5: " . $config->ipsendhex_step5 . "\n";
   		$hexResponse = &writeReadBuffer($config->ipsendhex_step5,$config->iprecvhex_step5);

	} else	
	{
	   #
   	   # Serial Request Inverter Version Information
   	   #
     	...
     	&parseVersData($hexResponse) if ($hexResponse ne "");
   	   ...
   	   #
   	   # Serial Request Inverter Parameter Format Information
   	   #
	   ...
	   #
	   # Serial Request Inverter Parameter Information
	   #
	   ...
	   #
	   # Serial Request Inverter Data Format Information
	   #
   ...
   while (1) {
     ...
     #
	if ($ipport) {
    	print "Send -> req data as at " . &getDateTime() . " : " . $config->ipsendhex_data . "\n";
    	&setPollTimes();
    	$hexResponse = &writeReadBuffer($config->ipsendhex_data,$config->iprecvhex_data);
	} else
	{
    ...
    &parseData($hexResponse) if ($hexResponse ne "");
     ...
     #
     # Write data to file if it is time to do so
     #
     if ( &isNextFileoutputDue() ) {
         print "File output as at " . &getDateTime() . " ...\n";
         &writeToFile();
         &setFileoutputTimes();
     }
     ...
     if ($config->flags_use_pvoutput) {
       if ( &isNextPvoutputDue() ) {
         ...
         if ($HoH{VPV}{VALUE}) {
         	$voltageAVG = $HoH{VPV}{VALUE} ;
         } else
         {
         	$voltageAVG = ($HoH{VPV1}{VALUE} + $HoH{VPV2}{VALUE}) / 2 ;
         }
         print "PVOUTPUT as at " . &getDateTime() . " ...\n";
         print "  ran: " . $config->scripts_pvoutput . " " . ($HoH{ETODAY}{VALUE} * 1000) . " $HoH{PAC}{VALUE} $date $time $HASH{SERIAL} $voltageAVG\n";
         system ($config->scripts_pvoutput . " " . ($HoH{ETODAY}{VALUE} * 1000) . " $HoH{PAC}{VALUE} $date $time $HASH{SERIAL} $voltageAVG" );
         &setPvoutputTimes();
       }
     }
